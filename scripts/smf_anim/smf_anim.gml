...
{	//Creates a new empty animation
	...
	keyframeNum = 0;
	lockedBonesList = ds_list_create();
	
	/// @func trim(scale)
	static trim = function(scale)
	{
		//Trims down the number of keyframes by the given scale
		var newGrid = ds_grid_create(2, newNum);
		var i = 0;
		var ind = 0;
		while (ind < keyframeNum)
		{
			newGrid[# 0, i] = keyframeGrid[# 0, floor(ind)];
			newGrid[# 1, i] = keyframeGrid[# 1, floor(ind)];
			ind += 1 / scale;
			i ++;
		}
	...
	{
		ds_grid_destroy(keyframeGrid);
		ds_list_destroy(lockedBonesList);
	}
	...
	static duplicate = function() 
	{
		...
		ds_list_copy(newAnim.lockedBonesList, lockedBonesList);
		
		//Duplicate the contents of the old animation's keyframe grid
	...
	{	/*	Finds the rig-space dual quaternion of the given node.
		...
		if !is_array(keyframe){
			smf_debug_message("Error in script keyframe_get_node_dq: Trying to read from non-existing keyframe " + string(keyframeInd)); 
			exit;}
	...
	{	//Writes a keyframe to a string, for copying more easily to clipboard
		var keyframe = keyframeGrid[# 1, keyframeInd];
		if !is_array(keyframe){
			smf_debug_message("Error in script anim_keyframe_get_string: Trying to read from non-existing keyframe " + string(keyframeInd)); 
			exit;}
		//Create string
	...
	{	//Overwrites a keyframe from a string
		...
		if probe != "KEYFRAME"
		{
			smf_debug_message("Error in script keyframe_set_from_string: The given string does not contain a keyframe"); 
			return false;
		}
	...
	{	/*	Creates a keyframe at the given time using the given sample as reference
		...
		for (var i = 0; i < nodeNum; i ++)
		{
			...
			if (smf_quat_dot(keyframe[i], deltaLocalDQ) < 0)
			{
				smf_dq_negate(deltaLocalDQ);
			}
	...
	static keyframe_set_node_dq = function(rig, keyframeInd, nodeInd, DQ, moveFromCurrent, transformChildren) 
	{
		...
		if !is_array(keyframe)
		{
			smf_debug_message("Error in script anim_keyframe_set_node_dq: Trying to modify non-existing keyframe " + string(keyframeInd)); 
			exit;
		}
	...
	static update_locked_bones_in_sample = function(rig, nodeInd, sample, transformChildren)
	{
		...
		for (var i = 0; i < num; i ++)
		{
			var ind = descendants[i];
			if (ds_list_find_index(lockedBonesList, ind) < 0){continue;}
			var cNode = nodeList[| ind];
...
function anim_write_to_buffer(saveBuff, anim)
{
	...
	//Write header
	buffer_write(saveBuff, buffer_string, "SnidrsAnimation v2");
	...
	//Write locked bones for this animation
	var lockedBonesNum = ds_list_size(anim.lockedBonesList);
	buffer_write(saveBuff, buffer_u16, lockedBonesNum);
	for (var i = 0; i < lockedBonesNum; i ++)
	{
		buffer_write(saveBuff, buffer_u16, anim.lockedBonesList[| i]);
	}
...
function anim_read_from_buffer(loadBuff)
{
	...
	if (header != "SnidrsAnimation v2")
	{
		version = 1;
		if (header != "SnidrsAnimation")
		{
			smf_debug_message("ERROR in script anim_read_from_buffer: Trying to read from a section that does not contain animation.");
			return -1;
		}
	}
