...
function smf_instance(_model) constructor 
{
	...
	/// @func fastSampleTest() 
	#macro SMFfastSampleTest if (fastSampling){smf_debug_message("Error in SMF instance: Can't edit fast-sampling instance!"); return true;}
	...
	static step = function(timeStep)
	{
		...
		if (currAnimInd < 0 && currAnimName != "")
		{
			...
			newAnimInd = model.animMap[? currAnimName] ?? -1;
		}
		...
		if (currAnimInd >= 0)
		{
			...
			else
			{
				...
				//Linearly interpolate between the previous and current samples
				if (animLerp < 1)
				{
	...
	{	/*	Play an animation in the given animation instance.
		...
		{
			smf_debug_message("Error in SMF instance's function \"play\": Could not find animation " + string(animName));
			return -1;
		}
	...
	/// @func node_rotate(node, xDegrees, yDegrees, zDegrees)
	static node_rotate = function(node, xDegrees, yDegrees, zDegrees)
	{	//Rotates a node around a custom rig-space axis
		SMFfastSampleTest
		sample_node_rotate(model.rig, node, sample, degtorad(xDegrees), degtorad(yDegrees), degtorad(zDegrees));
	}
	
	/// @func node_rotate_axis(node, degrees, ax, ay, az)
	static node_rotate_axis = function(node, degrees, ax, ay, az)
	{	//Rotates a node around a custom rig-space axis
		SMFfastSampleTest
