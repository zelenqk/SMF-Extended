...
function sample_lerp(S1, S2, amount, S = array_create(array_length(S1))) 
{	/*	Linearly interpolates between two samples.
	...
	if (abs(dp) < .5)
	{
		...
		repeat num div 8
		{
			...
			S[@ i++] = lerp(N1, M1 * s, amount);
			S[@ i++] = lerp(N2, M2 * s, amount);
			S[@ i++] = lerp(N3, M3 * s, amount);
			S[@ i++] = lerp(N4, M4 * s, amount);
			S[@ i++] = lerp(N5, M5 * s, amount);
			S[@ i++] = lerp(N6, M6 * s, amount);
			S[@ i++] = lerp(N7, M7 * s, amount);
		}
		return S;
	}
...
{	/*	
	...
	*/
	if (rig.boneNum == 0 || nodeInd >= rig.nodeNum || nodeInd < 0)
	{
		...
		smf_debug_message("ERROR in script sample_get_node_dq: Node " + string(nodeInd) + " does not exist");
		return targetQ;
	}
	var nodeList = rig.nodeList;
	var bindMap = rig.bindMap;
	...
	if (boneInd < 0)
	{
		...
		{
			smf_debug_message("ERROR in script sample_get_node_dq: The given node " + string(nodeInd) + " is not mapped to any bone and does not have any children.");
		...
		if (boneInd < 0)
		{
			smf_debug_message("ERROR in script sample_get_node_dq: The given node " + string(nodeInd) + " is not mapped to any bone.");
			array_copy(targetQ, 0, node[eAnimNode.WorldDQ], 0, 8);
			return targetQ;
		}
	}
...
{	/*	
		...
		This is useful for head turning and procedural animations.
		The node will rotate around its parent's position. If the node is not a bone, it will rotate around its own position.
		This script performs faster than if you were to rotate each axis separately
	*/
...
{	/*	
	...
	*/
	...
	{
		smf_debug_message("Error in sample_node_move_fast: Node does not exist in rig");
		exit;
	}
...
{	/*	
	...
	*///////////////////////////////////////////////////////////////
	...
	if (!cNode[eAnimNode.IsBone] || !pNode[eAnimNode.IsBone])
	{
		smf_debug_message("Error in script sample_node_ik: Cannot perform inverse kinematics on nodes that aren't bones"); 
		exit;
	}
...
{	/*	Snidr's Two-joint Inverse Kinematics Algorithm
	...
	*///////////////////////////////////////////////////////////////
	...
	if (!cNode[eAnimNode.IsBone] || !pNode[eAnimNode.IsBone])
	{
		smf_debug_message("Error in script sample_node_ik_fast: Cannot perform inverse kinematics on nodes that aren't bones"); 
		exit;
	}
