...
function smf_rig() constructor
{
	...
	static node_insert = function(nodeInd)
	{
		...
		if (nodeInd <= 0 || ds_list_size(nodeList) < 2){
			smf_debug_message("ERROR in script node_insert: Could not insert node at position " + string(nodeInd));
			exit;}
	...
	static node_get_dq = function(nodeInd) 
	{
		...
		if is_undefined(node){
			smf_debug_message("ERROR in script node_get_dq: Invalid nodeIndex " + string(nodeInd));
			return false;}
	...
	static node_delete = function(nodeInd) 
	{
		...
		if (nodeInd < 0){
			smf_debug_message("ERROR in script node_delete: Trying to delete a nodeInd < 0");
			return false;}
		...
		if nodeInd >= nodeNum{
			smf_debug_message("ERROR in script node_delete: Node " + string(nodeInd) + " does not exist in rig");
			return false;}
	...
	{	/*	This script can potentially mess up your rig structure. There are checks in place 
		...
		if (nodeInd == 0){
			smf_debug_message("ERROR in script node_set_parent: Cannot change the parent of the first node");
			return false;}
	
		//Exit if the node index doesn't make sense
		if (nodeInd < 0){
			smf_debug_message("ERROR in script node_set_parent: Trying to edit a nodeIndex < 0");
			return false;}
			
		//Exit if the node doesn't exist
		var node = nodeList[| nodeInd];
		if !is_array(node){
			smf_debug_message("ERROR in script node_set_parent: Node " + string(nodeInd) + " does not exist in rig");
			return false;}

		//Exit if the new parent's index is larger or equal to the current node
		if parent >= node[eAnimNode.Parent]{
			smf_debug_message("ERROR in script node_set_parent: Cannot assign parent that has a higher node index than the child");
			return false;}
	...
	static node_set_bone = function(nodeInd, enable) 
	{
		...
		if (nodeInd == 0){
			smf_debug_message("ERROR in script node_set_bone: Cannot make the first node in the hierarchy represent a bone");
			return false;}
	
		//Exit if the node index doesn't make sense
		if nodeInd < 0{
			smf_debug_message("ERROR in script node_set_bone: Invalid nodeIndex " + string(nodeInd));
			return false;}
	...
	static node_get_bone = function(nodeInd)
	{
		var node = nodeList[| nodeInd];
		if (is_undefined(node)){
			smf_debug_message("ERROR in script node_get_bone: Invalid node index " + string(nodeInd));
			return false;}
	...
	{	/*	Updates a node's orientation in relation to its parent. The following needs to be correct:
		...
		if (node[eAnimNode.IsBone])
		{
			...
			{
				smf_debug_message("Node " + string(nodeInd) + " has no up vector. DQ: " + string(nodeDQ));
			}
...
function rig_read_from_buffer(loadBuff) 
{
	...
	if (header != "Rig")
	{
		smf_debug_message("Error in script anim_rig_read_from_buffer: Trying to read from a section that does not contain a rig.");
		exit;
	}
